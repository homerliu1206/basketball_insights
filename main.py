...
Pdict = {"KobeBryant": 0, "JoeJohnson": 1, "LeBronJames": 2, "CarmeloAnthony": 3, "DwightHoward": 4,
         "ChrisBosh": 5, "ChrisPaul": 6, "KevinDurant": 7, "DerrickRose": 8, "DwayneWade": 9}
...
# Matrix
Games = np.array([KobeBryant_G, JoeJohnson_G, LeBronJames_G, CarmeloAnthony_G, DwightHoward_G,
                  ChrisBosh_G, ChrisPaul_G, KevinDurant_G, DerrickRose_G, DwayneWade_G])
...
MinutesPlayed = np.array(
    [KobeBryant_MP, JoeJohnson_MP, LeBronJames_MP, CarmeloAnthony_MP, DwightHoward_MP,
     ChrisBosh_MP, ChrisPaul_MP, KevinDurant_MP, DerrickRose_MP, DwayneWade_MP])
...
# Matrix
FieldGoals = np.array(
    [KobeBryant_FG, JoeJohnson_FG, LeBronJames_FG, CarmeloAnthony_FG, DwightHoward_FG, ChrisBosh_FG,
...
FieldGoalAttempts = np.array(
    [KobeBryant_FGA, JoeJohnson_FGA, LeBronJames_FGA, CarmeloAnthony_FGA, DwightHoward_FGA,
     ChrisBosh_FGA, ChrisPaul_FGA, KevinDurant_FGA, DerrickRose_FGA, DwayneWade_FGA])
...
# Matrix
Points = np.array([KobeBryant_PTS, JoeJohnson_PTS, LeBronJames_PTS, CarmeloAnthony_PTS, DwightHoward_PTS,
                   ChrisBosh_PTS, ChrisPaul_PTS, KevinDurant_PTS, DerrickRose_PTS, DwayneWade_PTS])
...
# Matrix
FreeThrows = np.array([KobeBryant_FT, JoeJohnson_FT, LeBronJames_FT, CarmeloAnthony_FT, DwightHoward_FT,
...
# Matrix
FreeThrowsAttempts = np.array([KobeBryant_FTA, JoeJohnson_FTA, LeBronJames_FTA, CarmeloAnthony_FTA, DwightHoward_FTA,
                               ChrisBosh_FTA, ChrisPaul_FTA, KevinDurant_FTA, DerrickRose_FTA, DwayneWade_FTA])

# Color
Col = {"KobeBryant": 'Red', "JoeJohnson": 'Orange', "LeBronJames": 'Yellow', "CarmeloAnthony": 'Green', "DwightHoward": 'Blue',
        "ChrisBosh": 'Red', "ChrisPaul": 'Orange', "KevinDurant": 'Yellow', "DerrickRose": 'Green', "DwayneWade": 'Blue'}

# Marker
Mrk = {"KobeBryant": 's', "JoeJohnson": 'o', "LeBronJames": '^', "CarmeloAnthony": 'D', "DwightHoward": 's',
        "ChrisBosh": 'o', "ChrisPaul": '^', "KevinDurant": 'D', "DerrickRose": 's', "DwayneWade": 'o'}


def ft_per_game(playerlist):
    """Free Throw Attempts per game"""
    for name in playerlist:
        plt.plot(FreeThrowsAttempts[Pdict[name]] / Games[Pdict[name]], c = Col[name], marker = Mrk[name], label = Players[Pdict[name]])
...
def accuracy_ft():
    ...
    for name in playerlist:
        plt.plot(FreeThrows[Pdict[name]] / FreeThrowsAttempts[Pdict[name]], c = Col[name], marker = Mrk[name], label = Players[Pdict[name]])
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.xticks(list(range(10)), Seasons, rotation='vertical')
    plt.show()


def playing_style(playerlist):
    """Player playing style (2 vs 3 points preference) excluding Free Throws"""
    for name in playerlist:
        plt.plot(
            (Points[Pdict[name]] - FreeThrows[Pdict[name]])/FieldGoals[Pdict[name]], c = Col[name], marker = Mrk[name], label = Players[Pdict[name]])
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1))
    plt.xticks(list(range(10)), Seasons, rotation='vertical')
    plt.show()
